...
func cacheFedEvent(ev share.TLogEvent, msg, fullname, remote, session string, roles map[string]string) {
...
func promoteToMaster(w http.ResponseWriter, acc *access.AccessControl, login *loginSession, reqData api.RESTFedPromoteReqData) (
	...
	cacheFedEvent(share.CLUSEvFedPromote, msg, login.fullname, login.remote, login.id, login.domainRoles) // The error is handled within the function.
...
func demoteFromMaster(w http.ResponseWriter, acc *access.AccessControl, login *loginSession) (
	...
	cacheFedEvent(share.CLUSEvFedDemote, "Demote from primary cluster", login.fullname, login.remote, login.id, login.domainRoles) // The error is handled within the function.
	evqueue.Flush()
	revertFedRoles(acc)
	cleanFedRules()
	...
	if err == nil {
		...
		if err = json.Unmarshal(data, &respTo); err == nil {
			...
			if err = clusHelper.PutFedMembership(&membership); err == nil {
				...
				cacheFedEvent(share.CLUSEvFedJoin, msg, login.fullname, login.remote, login.id, login.domainRoles) // The error is handled within the function.
...
func leaveFed(w http.ResponseWriter, acc *access.AccessControl, login *loginSession, req api.RESTFedLeaveReq,
	...
	if bodyTo, err := json.Marshal(&reqTo); err == nil {
		...
		if err == nil || req.Force {
			...
			if err := clusHelper.PutFedMembership(&membership); err == nil {
				cacheFedEvent(share.CLUSEvFedLeave, "Leave federation", login.fullname, login.remote, login.id, login.domainRoles) // The error is handled within the function.
				evqueue.Flush()
				if w == nil {
					// called by configmap
...
func handlerRemoveJointCluster(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if status != http.StatusOK {
		...
		cacheFedEvent(share.CLUSEvFedKick, msg, login.fullname, login.remote, login.id, login.domainRoles) // The error is handled within the function.
...
func handlerJoinFedInternal(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
			...
			if reqData.JointCluster.K8sUID != "" && _allowSameK8sUidRejoin {
				...
				for id := range cacher.GetFedJoinedClusterIdMap(accReadAll) {
					...
					if joinedCluster.Name == reqData.JointCluster.Name && joinedCluster.K8sUID == reqData.JointCluster.K8sUID {
						...
						if status == http.StatusOK {
							msg := fmt.Sprintf("Dismiss cluster %s(%s) from federation", joinedCluster.Name, joinedCluster.RestInfo.Server)
							cacheFedEvent(share.CLUSEvFedKick, msg, reqData.User, reqData.Remote, "", reqData.UserRoles)
						}
						break
					}
				}
	...
	if err = clusHelper.PutFedJointClusterList(list); err == nil {
		...
		cacheFedEvent(share.CLUSEvFedJoin, msg, reqData.User, reqData.Remote, "", reqData.UserRoles) // The error is handled within the function.
	...
	if joinedCluster.ID == req.ID {
		...
		if err := jwtValidateFedJoinTicket(req.JointTicket, joinedCluster.Secret); err == nil {
			if status, code = removeFromFederation(&joinedCluster, accReadAll); status == http.StatusOK {
				...
				cacheFedEvent(share.CLUSEvFedLeave, msg, req.User, req.Remote, "", req.UserRoles) // The error is handled within the function.
...
func handlerJointKickedInternal(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	cacheFedEvent(share.CLUSEvFedKick, "Dimissed from federation", userName, login.remote, login.id, login.domainRoles) // The error is handled within the function.
	evqueue.Flush()
	go leaveFedCleanup(masterCluster.ID, jointCluster.ID, false)
...
func workFedRules(fedSettings *api.RESTFedRulesSettings, fedRevs map[string]uint64, localRevs map[string]uint64, acc *access.AccessControl) bool {
	...
	if updated {
		cacheFedEvent(share.CLUSEvFedPolicySync, "Sync up policy with primary cluster", "", "", "", nil) // The error is handled within the function.
		...
		if strings.Index(request, "/v1/fed/") == 0 {
			...
			if method == http.MethodPost {
				...
				if importURIs.Contains(reqPath) {
					query := restParseQuery(r)
					if scope, checkErr := checkScopeParameter(w, query, share.ScopeLocal, enumScopeLocal); scope != share.ScopeLocal || checkErr != nil {
